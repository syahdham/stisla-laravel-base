...
<div class="form-group row mb-4">
    <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ $label }}<span class="text-danger">{{ $required != '' ? '*' : '' }}</span></label>
    <div class="col-sm-12 col-md-5">
        <select class="form-control js-example-basic-single" name="{{ $name }}" {{ $required }}>
            <option value="">- Pilih {{ $label }} -</option>
            @foreach ($values as $value)
                <option value="{{ $value->id }}" {{ $value->id == $selected ? 'selected' : old($name) ? 'selected' : '' }}>{{ $value->name }}
                </option>
            @endforeach
        </select>
    </div>
